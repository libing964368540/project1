            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.DataCount == 0) {
                        ...
                        var data = $.parseJSON(data.DataList)
                        $.each(data, function (i, v) {
                            if (v.FileName === null) {
                                v.FileName = "视频资料"
                            }
                            $('<a href="showPage.html?artId=' + v.ClassifyId + '&articleId=' + v.ArticleId + '"> <span class="title" title="' + v.ArticleTitle + '">' + v.ArticleTitle + ' </span> <span class="user">' + v.ArticleAuthor + '</span> <span class="date">' + ChangeDateTime(v.ArticleTime, 2) + '</span> </a>').appendTo($noticeCon)
                        })
                        //初始化分页
        ...
        function page(num, pageBox) {
            ...
            $body.find("#pages #Pagination").pagination(num, {
                    ...
                    callback: function (current_page) {
                        pageConList(dataId, current_page + 1, 10)
                    }
                }
            )
        }
